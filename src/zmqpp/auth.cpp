...
// MAJOR TODO Introduce logging hook!
// MAJOR TODO Use command->delegate hashmap to handle commands! 
namespace zmqpp
{
auth::auth(context& ctx) :
  ...
  verbose(false)
  {
    ...
    authenticator = std::make_shared<actor>(
        std::bind([this] (socket * pipe, context& auth_ctx) -> bool {
                    // spawn ZAP handler
...
void auth::handle_command(socket& pipe) {
	...
	std::string command = msg.get<std::string>(0);
	if (verbose) {
    	std::cout <<"auth: API command=" << command << std::endl;
    }

    if ("ALLOW" == command) {
    	std::string address = msg.get<std::string>(1);
    	if (verbose) {
    	...
    	std::string domain = msg.get<std::string>(1);
    	if (verbose) {
    	...
    	std::string user = msg.get<std::string>(1);
    	std::string pass = msg.get<std::string>(2);
    	...
    	std::string client_public_key = msg.get<std::string>(1);
    	...
    	std::string verbose_string = msg.get<std::string>(1);

    	verbose = "true" == verbose_string;
    	pipe.send(signal::ok); 
